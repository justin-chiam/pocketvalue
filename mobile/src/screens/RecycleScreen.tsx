...
export function RecycleScreen({ blurb, model, onClose }: Props) {
  ...
  return (
    <RecommendationDetail title="Recycle" blurb={blurb} icon={HandHeartIcon} onClose={onClose}>
      <View style={styles.section}>
        <Text style={styles.sectionLabel}>Recycling centers near you</Text>
        {state.status === 'loading' ? (
          <View style={styles.loadingRow}>
            <ActivityIndicator size="small" color={colors.pine} />
            <Text style={styles.note}>Finding recycling spots near you…</Text>
          </View>
        ) : state.status === 'denied' ? (
          <Text style={styles.note}>
            Allow location access to see places near you that can recycle e-waste.
          </Text>
        ) : state.status === 'error' ? (
          <>
            <Text style={styles.note}>Couldn’t find recycling spots right now.</Text>
            <TouchableOpacity
              accessibilityRole="button"
              accessibilityLabel="Retry finding recycling spots"
